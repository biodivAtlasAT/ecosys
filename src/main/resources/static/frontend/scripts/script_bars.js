                ...
                if (reqEcosys.length > 0) {
                    $.ajax({
                        ...
                        success: function (resp) {
                            ...
                            func_initChart(resp['data'], points);
                        }
                    });
                }
            }
...
func_initChart = function (data, m_points) {
    if ($(".cl_svg").length > 0) {
        $(".cl_svg").remove();
    }
    filteredmarker = marker.filter(x => x !== undefined);
    if (filteredmarker.length > 1) {
        ...
        for(it_d = 0; it_d < data.length; it_d++) {
            ...
            for (it_n = 0; it_n < data[it_d]['vals'].length; it_n++) {
                yStep[it_n] = (data[it_d]['vals'][it_n]['val']); //
            }
...
// Y axis
            ...
            // Add Y axis
            for(it_n = 0; it_n < data[it_d]['vals'].length; it_n++) {
                x0 = xScale(xStep[it_n]) + margin.left;
                y0 = yScale(yStep[it_n]) - margin.top;
                x1 = xScale(xStep[it_n]) + margin.left;
