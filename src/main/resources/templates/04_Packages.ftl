            ...
            <div class="md-col-10">
                <table class="table table-striped" id="packageTable">
                    ...
                    <tr>
                        ...
                        <th scope="col" style="text-align:left">Ausgewählt</th>
                        <th scope="col">&nbsp;</th>
                        <th scope="col">&nbsp;</th>
                    </tr>
                    </thead>
                    ...
                    <#list result as prop>
                        <tr>
                            ...
                            <td style="text-align:center">
                                <input type="radio" id="rb_${prop.id}" value="${prop.id}" name="default" <#if prop.default == true>checked<#else></#if> disabled>
                            </td>
                            <td>
                                <button id="bt_${prop.id}" class="btn btn-sm btn-outline-primary" style="margin-top:-4px" role="button" onclick='packageEdit("${prop.id}", ${maxCount}, "${wordSave}", "${wordEdit}");'>${wordEdit}</button>
                            </td>
                            <td>
                                <button id= "btDel_${prop.id}" class="btn btn-sm btn-outline-danger" style="margin-top:-4px" role="button" onclick='packageDelete("${prop.id}", <#if idsInUse?seq_contains(prop.id) || prop.default == true>true<#else>false</#if>);'>${wordDelete}</button>
                            </td>
                        </tr>
                    </#list>
                    <tr>
                        ...
                        <td>
                            <button id="bt_-1" style="margin-top:-4px" class="btn btn-sm btn-primary funcSave" role="button" onclick='packageEdit("-1", ${maxCount}, "${wordSave}", "${wordEdit}");'>${wordSave}</button>
                        </td>
                        <td>&nbsp;
                        </td>
                    </tr>
...
<form id="packageSave" onclick="packageSave()" action="./packageUpdate" method="post">
    <input type="hidden" id="saveFormName" name="name">
    <input type="hidden" id="saveFormId" name="id" value="-1">
    <input type="hidden" id="saveFormDefault" name="default" value="false">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="document.getElementById('packageSave').submit();return true;">Löschen</button>
            </div>
        </div>
    </div>
</div>
